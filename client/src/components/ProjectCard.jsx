...
export default function ProjectCard({
  ...
  return (
    <Link
      to={`/projects/${id}`}
      className="w-[80vw] bg-white border border-zinc-900 space-y-8 lg:space-y-4 rounded-lg p-4 my-4 hover:bg-zinc-100 transition-colors"
    >
      <h2 className="text-xl font-semibold">{projectTitle}</h2>
      <div className="w-full   px-4  lg:w-2/3 flex flex-col lg:flex-row  items-center lg:items-end  justify-start lg:justify-between  space-y-4 lg:space-x-4 ">
        <p className="font-medium flex items-center justify-between lg:justify-center w-full text-md text-zinc-600   ">
          Total Budget:
